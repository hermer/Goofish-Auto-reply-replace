            ...
            try:
                ...
                if current_time - self.last_token_refresh_time >= self.token_refresh_interval:
                    ...
                    if new_token:
                        ...
                        # 优先尝试一次强制刷新Cookie（CookieCloud -> 浏览器），仅在本次失败时触发一次
                        forced_refreshed = False
                        try:
                            host = os.getenv("COOKIE_CLOUD_HOST") or ""
                            uuid = os.getenv("COOKIE_CLOUD_UUID") or ""
                            pwd = os.getenv("COOKIE_CLOUD_PASSWORD") or ""
                            if host and uuid and pwd:
                                try:
                                    from utils.cookiecloud import fetch_cookiecloud_cookie_str  # type: ignore
                                except Exception:
                                    fetch_cookiecloud_cookie_str = None
                                if fetch_cookiecloud_cookie_str:
                                    try:
                                        new_cookies_str = await fetch_cookiecloud_cookie_str(host, uuid, pwd)
                                        if new_cookies_str:
                                            self.cookies_str = new_cookies_str
                                            self.cookies = trans_cookies(new_cookies_str)
                                            await self.update_config_cookies()
                                            logger.info(f"【{self.cookie_id}】已通过CookieCloud强制刷新Cookie，尝试重新获取Token...")
                                            forced_refreshed = True
                                    except Exception as fe:
                                        logger.error(f"CookieCloud 强制刷新异常: {self._safe_str(fe)}")
                        except Exception as e2:
                            logger.error(f"强制刷新Cookie异常: {self._safe_str(e2)}")

                        if not forced_refreshed:
                            try:
                                logger.info(f"【{self.cookie_id}】尝试通过浏览器强制刷新Cookie...")
                                ok = await self._refresh_cookies_via_browser()
                                forced_refreshed = bool(ok)
                            except Exception as be:
                                logger.error(f"浏览器强制刷新Cookie异常: {self._safe_str(be)}")

                        # 若强制刷新成功，立即重试一次获取Token
                        if forced_refreshed:
                            retry_token = await self.refresh_token()
                            if retry_token:
                                logger.info(f"【{self.cookie_id}】强制刷新Cookie后Token获取成功，准备重启实例...")
                                self.connection_restart_flag = True
                                if self.ws:
                                    await self.ws.close()
                                break
